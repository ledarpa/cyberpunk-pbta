# Chapería

## Qué es la chapería

Chapería es el equipamiento que no va implantado: armas, armaduras, herramientas, drones, ropa táctica, datos en neuroranura, etc. En la jerga de la calle: si no está clavado en el cuerpo, es chapería.

---

## Cromos vs chapería

| Aspecto | Cromos |
| --- | --- |
| Dónde va | Integrado al cuerpo |
| ¿Afecta @Psique / humanidad? | **Sí** |
| Variedad | Menos piezas; cada una pesa mucho |
| Uso en juego | Capacidades «instintivas» del implante |

| Aspecto | Chapería |
| --- | --- |
| Dónde va | Inventario, vehículo, bolsillos |
| ¿Afecta @Psique / humanidad? | **No** |
| Variedad | Mucha variedad; más accesible |
| Uso en juego | Hay que saber manejar la pieza; llevar arsenal llama la atención |

En resumen: Armas, herramientas, neurodata en ranura y vestimenta:

Muchas piezas piden conocimiento técnico o integración con cromos (HUD + cyberópticas). Llevar arsenal letal o PEM a la vista puede ser contraproducente: se resuelve en ficción con el Director.

---

## Recordatorio rápido

- La chapería no suma @Psique.

- Pistola de munición estándar: legal en Night City; el resto exige permiso o ir ilegal (lo habitual).

- SAI = sistema de arma inteligente (hace falta el cromo de conexión de arma inteligente, salvo que el arma diga otra cosa).

- Accesorios y subsistemas: se añaden sin penalidad; solo pagan su coste en Créditos/Estrellas.

### SAI (se repiten en varias armas)

| SAI | Efecto |
| --- | --- |
| Acelerador iónico | Proyectiles/energía: **+1 daño** por impacto (rifle y lanzamisiles: **+3** donde el arma lo indique) |
| Apuntado asistido | Con HUD visual: info del objetivo en tiempo real. **Ventaja en RC** |
| Flujo balístico | Sin tiempos de recarga (munición movilizada por el SAI) |
| Estabilizador | Arma usable a **una mano** (fusil; rifle usa «estabilizador y decompresor») |

---

## Armas

### Pistola

| Stat | Valor |
| --- | --- |
| Cargas | 15 (balas o celdas) |
| Alcance | 60 m |
| Manual | 1 tiro/acción, **1d6** daño |
| Semi | Ráfaga 3 tiros, RC con **ventaja**, **1d6** daño |

Tecnologías: pólvora/percutor, láser, plasma, proyectil o rayo (a elección de ficción).

- Hasta 1 pistola por brazo (efectos multiplicados).

- SAI por calidad: Improvisada: sin SAI · Corriente: 1 subsistema · Hi-Tech: 2 · Militar: 3

Accesorios

- Cargador ampliado: 21 cargas

- Silenciador: disparos sin sonido

SAI: acelerador iónico, apuntado asistido, flujo balístico (tabla arriba).

---

### Escopeta

| Stat | Valor |
| --- | --- |
| Cargas | 6 |
| Alcance | 10 m |
| Área | Cono **45°**, **1d6** por enemigo impactado |
| Quemarropa (≤3 m) | **3d6** a un solo objetivo |

Requisito: 2 manos o 2 cyberextremidades, salvo SAI estabilizador.

SAI por calidad: igual que pistola (Improvisada sin SAI · 1 / 2 / 3 subsistemas).

Accesorios

- Cañón largo: dispersión 23° (mitad); alcance ×2 (no integra con cyberópticas)

- Cañón doble: 2 proyectiles a la vez → daño ×2

- Cargador ampliado: 10 cargas

- Estabilizador: una mano

SAI: acelerador iónico, apuntado asistido, flujo balístico.

---

### Fusil

| Stat | Valor |
| --- | --- |
| Cargas | 30 |
| Alcance | 30 m |
| Manual | 1 tiro/acción, **1d6** |
| Semi | 3 tiros, RC con **ventaja**, **1d6** |
| Auto | Vacía cargador; cono **45°**; RC para todos en el radio; **1d6 por cada éxito** |

Requisito: 2 manos o 2 cyberextremidades, salvo SAI estabilizador.

SAI por calidad: Improvisada sin SAI · Corriente 1 · Hi-Tech 2 · Militar 3 subsistemas.

Accesorios

- Mira telescópica: alcance ×2; con cyberóptica con zoom: ×3

- Cargador ampliado: 45 cargas

- Silenciador: sin sonido

SAI: acelerador iónico (+1), apuntado asistido, estabilizador, flujo balístico.

---

### Rifle

| Stat | Valor |
| --- | --- |
| Cargas | 4 |
| Alcance | 100 m |
| Daño | **3d6** |
| Cadencia | 1 disparo; turno descarga + turno carga (**1 disparo cada 2 turnos**) |

Requisito: 2 manos o 2 cyberextremidades.

SAI por calidad: Improvisada sin SAI · 1 / 2 / 3 subsistemas.

Accesorios

- Mira telescópica: alcance ×3; con zoom óptico integrado: ×4

- Cargador ampliado: 7 cargas

- Silenciador: sin sonido; daño baja a 2d6; −50 m de alcance

SAI

- Acelerador iónico: +3 daño

- Apuntado asistido: ventaja RC

- Estabilizador y decompresor: estabilización + expulsar casquillo/celda → disparar todos los turnos

- Flujo balístico

---

### Lanzadardos

| Stat | Valor |
| --- | --- |
| Cargas | 1 |
| Cadencia | Turno disparo + turno recarga |
| Alcance | 15 m |
| Daño | **1 punto** con **10+**; **7–9** sin daño |
| Otro | Silenciosa |

Lanza dardos cibernéticos o químicos.

SAI por calidad: Improvisada sin SAI · 1 / 2 / 3 subsistemas.

SAI

- Acelerador iónico: alcance ×2, +1 daño

- Apuntado asistido: ventaja RC

- Flujo balístico: 1 dardo por turno sin tiempo de recarga

---

### Lanzamisiles

| Stat | Valor |
| --- | --- |
| Cargas | 1 |
| Alcance | 150 m |
| Daño | **4d6** |
| Cadencia | Disparo cada **2 turnos** (disparo + descarga + carga) |

Requisito: 2 manos o 2 cyberextremidades, salvo accesorio Minimal.

SAI por calidad: Improvisada sin SAI · 1 / 2 / 3 subsistemas.

Accesorios

- Minimal: versión reducida — 2d6, 50 m, una mano o una cyberextremidad

- Lanzagranadas: lanza granadas en lugar de misiles, +1d6 daño; con Minimal, granadas sin esa bonificación

- Recámara doble: 2 misiles al mismo objetivo en un disparo; efectos combinados

SAI

- Mira para proyectiles: con HUD, curvar trayectoria del misil; interfaz en arma o implante. Ventaja RC

- Flujo balístico: sin tiempos de recarga

- Acelerador iónico: +3 daño

---

### Granadas

Se venden en cajas de 8 unidades.

| Tipo | Efecto |
| --- | --- |
| **Ácida** | **1d6** corrosivo, radio **1 m**. Si el daño es **5+**, destruye un **segundo** cromo (automático **−2** a tiradas; marca daño en zona «*») o chapería a elección del lanzador |
| **Aturdidora** | Anula oído orgánico, cyberoído y radar ultrasónico **1d6** turnos, radio **10 m**. Turno de detonación: quien no supere **TM** pierde la acción. Atenuador de dB anula |
| **Criogénica** | **1d6** congelante, radio **3 m**; **1d6** turnos a mitad de movimiento; congela/destruye tejido vivo; paraliza temporalmente cromos y chapería |
| **Fragmentación** | **3d6** explosivo, radio **5 m** |
| **Gas** | Nube tóxica **5 m**, **1d6** turnos; **1d6** daño por turno dentro del radio |
| **Humo y partículas** | Anula visión orgánica, cyberóptica y radar EM **1d6** turnos, radio **5 m**. Respirar: **6−** en TM pierde el primer turno. Filtro de toxinas anula lo respiratorio |
| **PEM** | Anula cibernética y electrónica en **5 m** durante **1d6** turnos |

---

### Balística especial

Todas las armas de fuego o energía incluyen munición estándar. Munición especial: 4 cargadores = 1 Estrella (1 Crédito).

#### Proyectiles y celdas de energía

| Tipo | Efecto |
| --- | --- |
| Balas de carbono | Pistola y fusil **+1** daño; escopeta y rifle **+3** |
| Celdas entrelazado cuántico | Disparos atraviesan al primer objetivo (rifle: hasta **3** objetivos) |
| Celdas sobrecargadas | Disparar **2 veces** al mismo objetivo en el turno (segundo disparo: RC con **−1**) |
| Proyectiles de goma y descarga | Daño **no letal** a tejido biológico y cibernética |
| Rastreador | Sin daño; **1** proyectil por cargador (resto del cargador = transmisor/receptor) |

#### Misiles

| Tipo | Efecto |
| --- | --- |
| Explosivo | **+1d6** daño, radio **3 m** |
| Criogénico | Supervivientes congelados **1d6/2** turnos, radio **3 m**; mitad de acciones |
| PEM | Anula cibernética/electrónica en **15 m** durante **2d6** turnos |
| Teletaladro | Tras impacto, taladro a 90° + corrosión; en **1d6+1** turnos, agujero para que entre **una persona** |

#### Dardos (lanzadardos)

Cualquier dardo puede venderse como jeringa: impacto cuerpo a cuerpo, tirada TM en lugar de RC.

| Dardo | Efecto |
| --- | --- |
| **@Psico** | Episodio de cyberpsicosis **1d6** turnos si el blanco tiene **algún cromo**. Sin cromos: inmune |
| **Alucinógeno** | Desventaja en **todas** las tiradas **1d6** turnos |
| **Nanocontrolador** | Nanobots controlan cyberware del afectado (y artefactos). Sin cromos: inmune |
| **Veneno letal** | Muerte en **6** turnos si un biohacker no lo trata. No afecta cyberware |
| **Paralizante** | Inmovilizado **1d6** turnos |

---

## Herramientas

### Autointérprete

Presentación variable (instrumento, caja de sonido, etc.). Colocado en un punto; control remoto. Al activar, reproduce voces o música creíble para atraer atención.

### Drone

| Calidad | Subsistemas |
| --- | --- |
| Improvisado | 1 |
| Corriente | 2 |
| Hi-Tech | 3 |
| Militar | 4 |

- Movimiento: 5 m/turno
- Vida: 5 puntos de daño

| Subsistema | Efecto |
| --- | --- |
| Centinela | Arma de celdas: **1d6**/turno |
| Kamikaze | Autodestrucción: **3d6** fuego + PEM **1d6** turnos |
| Ligero | Velocidad **×2** |
| Protocolo | Transceptor para hackeo avanzado vía dron |
| Robusto | **10** puntos de daño |
| Silencioso | Casi inaudible; solo en silencio total |
| Vigilante | Cámara/micrófono + almacenamiento; enlace a HUD de implantes si lo tienes |

### Emulador biométrico

Conecta a terminal de acceso; burla reconocimiento biométrico.

### Kit de conectividad

Usuario <-> terminal; ventaja EN en la red. 12 h de recarga antes de reutilizar el kit.

| Calidad | Conexión / usos |
| --- | --- |
| Improvisado | **3** turnos para conectar; **2** usos |
| Corriente | **1** turno; **3** usos |
| Hi-Tech | Extensión del cromo de conexión neuronal: sin turno extra de conexión; **4** usos |
| Militar | Además herramientas inalámbricas en terminales no vivas; **6** usos |

### Kit de primeros auxilios

Ventaja EN para curar tejido biológico. Un uso puede quitar efectos de toxinas en lugar de curar.

| Calidad | Curación / usos |
| --- | --- |
| Improvisado | **1d6**; **2** usos |
| Corriente | **1d6+3**; **3** usos |
| Hi-Tech | **2d6+3**; **4** usos |
| Militar | **2d6+6**; **6** usos |

### Kit de reparaciones

Ventaja EN para reparar cyberware implantado, portado, vehículos, etc. Un uso puede quitar anulación de sistemas en lugar de reparar.

El kit de calidad X puede reparar piezas hasta la categoría indicada (penalizador entre paréntesis = umbral máximo de pieza que repara bien; texto original del manual).

| Calidad del kit | Repara hasta (Impro / Corr / Hi-T / Mil) | Usos |
| --- | --- | --- |
| Improvisado | +0 / −1 / −2 / −3 | 2 |
| Corriente | +1 / +0 / −1 / −2 | 3 |
| Hi-Tech | +2 / +1 / +0 / −1 | 4 |
| Militar | +3 / +2 / +1 / +0 | 6 |

### Pistola garfio

Gancho por presión o imán; cable retrae (objetivo hacia ti o tú hacia el punto, según peso). Alcance 50 m. Puede capturar personas.

### Torreta móvil

Lista blanca precargada; dispara al enemigo más cercano: 1d6/turno. Tirada de actuación de la torreta: 2d6+0.

### Trauma card

Membresía Trauma Team en el bolsillo; monitoriza biológico y cibernético. Si caes, llega transporte aéreo con biohackers y escolta militar.

- Pago mensual; cada activación gasta la tarjeta (hay que renovar membresía).

| Nivel | Servicio |
| --- | --- |
| **Cliente** | Llega en **15 min**; te dejan en el domicilio de la suscripción tras estabilizar |
| **Miembro** | **5 min** |
| **Preferencial** | Tras **1 día** de terapia: curado y reparado al **50%** |
| **VIP** | Cubren **100%** de gastos y tiempo de tratamiento necesario (salud y cibernética) |

---

## Neurodata (contenido de ranura)

Se inserta en neuroranuras (cromo). Distinto de la Neurodata que vendes por Estrellas (experiencia de sesión).

### Base de datos

Contenido variable: código, redes, técnicas de combate para asistir cyberware, VIP, audio, etc. Ventaja en EN, MC, RC o TM cuando la tarea use esos datos (hackeo, emular voz/rostro, discurso, aguantar respiración, etc.).

### Memoria en blanco

Espacio para guardar datos que descargues en juego.

### Neuroexperiencia

Grabación tasada de una vivencia; moneda de intercambio (las propias valen más). Uso de aprendizaje o recreación.

### Protocolo velo

Anula la desventaja al entrar en la antigua red y enlaces remotos.

- Implementarlo es muy difícil incluso para netrunners veteranos.

- Si fallas al implementarlo, sigues expuesto a las IA de la red antigua.

- Doble 1: el protocolo se desactiva; IA pueden minar/invasar el procesador cerebral → consecuencias catastróficas (Director).

---

## Vestimenta

### Corposuit

Traje de alta gama; oculta cyberware y armas pequeñas.

| Calidad | Efecto |
| --- | --- |
| Improvisado | Solo ocultación |
| Corriente | **Ventaja MC** |
| Hi-Tech | Placas interiores: **+1 TM** solo para absorber daño físico |
| Militar | Traje experimental: al activar **protocolo**, pierde capacidades anteriores y gana modo militar (variantes abajo) |

Modos (solo Militar, tras activar protocolo)

| Modo | Efecto |
| --- | --- |
| **Ghost** | Camuflaje fotoreflectivo; casi invisible **1 h** o hasta apagar. **3** usos; recarga **2 h** por uso |
| **VIP box** | Armadura bloque macizo; cámaras/sensores miran fuera; **sin movimiento**; protege de **todo** daño. Recarga **6 h** tras uso |
| **Rapaz** | Magnetopacks en manos/pies: vuelo controlado **1 h**. **3** usos; recarga **2 h** por uso |

### Máscara fantasma

Irreconocible para sistemas ópticos (físicos u holográficos). Ventaja MC vs reconocimiento facial. No engaña a quien tenga la misma capacidad.

### Tecnoarmadura

Servomotores y placas.

| Calidad | Efecto |
| --- | --- |
| Improvisada | **Ventaja TM** para absorber cualquier daño (físico, químico, energético…) |
| Corriente | Además **+1 TM** |
| Hi-Tech | Además absorbe **1 nivel** de daño automáticamente |
| Militar | Absorbe **2 niveles** de daño automáticamente |

---
